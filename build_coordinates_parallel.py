...
def main(argv):
  ...
  # if rank == 0:
  #   all_coord = []
  indice_groups = np.array_split(indices, comm.size, axis=0)
  for i, coord_idx in tqdm(indice_groups[rank]):
    z, y, x = np.unravel_index(coord_idx, vol_shapes[i])

    coord = tf.train.Example(
  ...
  all_coord = comm.gather(coord_list, root=0)
  # comm.Gather(coord_list, all_coord, root=0)
  ###


  if rank == 0:
    print(len(all_coord))
    all_coord = reduce(operator.add, all_coord)
    # print(all_coord)

    # print(len(all_coord))
    with tf.python_io.TFRecordWriter(FLAGS.coordinate_output,
                                    options=record_options) as writer:
      for coord in tqdm(all_coord):
